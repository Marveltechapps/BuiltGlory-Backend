...
sequenceDiagram
    ...
    Backend->>Backend: Generate 6-digit OTP, set expiry 5 min
...
| Body | `{ "mobileNumber": "9876543210", "otp": "123456" }` — `enteredOTP` may be used in place of `otp`. |
| Validation | Both `mobileNumber` and a 6-digit numeric `otp` (or `enteredOTP`) required. |
...
- **OTP format:** 6-digit numeric, range 100000–999999. Generate with: `Math.floor(100000 + Math.random() * 900000).toString()`.
- **OTP expiry:** 5 minutes from generation. Store as `otpExpiry`; on verify, reject if `new Date() > new Date(user.otpExpiry)`.
- **Test number (optional):** For a specific test mobile number (e.g. `9698790921`), you may return a fixed 6-digit OTP (e.g. `123456`) instead of sending SMS. Document the values if you keep this.
...
- [ ] Implement `generateOTP()` returning a 6-digit string (100000–999999).
